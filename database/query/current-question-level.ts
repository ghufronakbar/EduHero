...
import { DB_NAME } from '../index'; // Sesuaikan path
...
type QuestionCategory = "matematika" | "fisika" | "kimia" | "biologi" | "inggris";
...
export const currentQuestionLevel = {
     ...
     */
    getNextDifficulty: async (category: QuestionCategory): Promise<Difficulty> => {
        ...
        const history = await db.getAllAsync<{ is_correct: number; difficulty: string }>(
            `
      ...
      `,
        ...
        console.log(`[Adaptive] Category: ${category} | Last 10 Score: ${totalScore}`);
    ...
    getAdaptiveQuestion: async (category: QuestionCategory) => {
        ...
        const targetDifficulty = await currentQuestionLevel.getNextDifficulty(category);
